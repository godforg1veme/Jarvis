...
- VPN Supervisor работает для VPS в Германии и Нидерландах: Host Agent
  ...
  read-only снимок состояния того же узла. Высокоуверенная рекомендация
  уведомляет владельца, но реальные repair-playbook’и и автоматический
  ремонт выключены. `/vpn_health` показывает состояние обоих узлов;
