...
- Telegram-бот с allowlist, пользовательской изоляцией, дедупликацией обновлений,
  ...
  последствиями подтверждаются отдельными inline-кнопками. Серверная часть
  кнопочной навигации и миграция состояния ввода развёрнуты на VPS 2026-09-14;
  ручная проверка раскладки из реальных owner/member Telegram-аккаунтов остаётся;
...
- Life OS v2: подключение выбранных внешних провайдеров вместо fixture transport,
  ...
  Desktop и обычные ответы/напоминания в реальном Telegram остаются ручными или
  внешними этапами; сервер v2 и PostgreSQL-схема уже развёрнуты.
