...
- первый слой VPN Supervisor развёрнут 2026-09-15: Host Agent детерминированно
  классифицирует одну первопричину сбоя, сервер строго сверяет диагноз с
  измерениями и открывает инцидент только после трёх одинаковых наблюдений.
  Telegram `/vpn_health` показывает тот же безопасный диагноз. Автоматический
  ремонт и вызов LLM пока намеренно отключены;
