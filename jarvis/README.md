...
- на VPS развёрнут выделенный Xray VPN для Happ: VLESS + REALITY + XTLS Vision
  на 443/8443 и изолированный fallback-сервис Hysteria 2 на DE
  (`87.120.187.109:443/udp`, SNI `vpn.rilora.ru`); отдельный NL-узел выдаётся
  по `94.183.208.56:20000-50000/udp` с тем же текущим SNI. Продление
  NL-сертификата требует отдельного DNS-имени; проверка трафика в Happ на
  реальном телефоне ещё не завершена.
...
- VPN Supervisor работает для VPS в Германии и Нидерландах: Host Agent
  детерминированно классифицирует первопричину, сервер сверяет диагноз и
  открывает инцидент после трёх одинаковых наблюдений. Изолированная LLM
  анализирует очищенные журналы и может запросить один дополнительный
  read-only снимок состояния того же узла. При подтверждённом сбое одного
  сервиса AI может предложить перезапуск только Xray или Hysteria2; действие
  выполняется после подтверждения владельца в личном Telegram и повторной
  проверки диагноза. Код развёрнут, но реальный перезапуск при инциденте ещё
  не проходил приёмку. `/vpn_health` показывает состояние обоих узлов;
  клиентские внешние protocol-probe реализованы как отдельный контур, но пока
  не включены: нужны отдельные тестовые ключи с подтверждением владельца;
