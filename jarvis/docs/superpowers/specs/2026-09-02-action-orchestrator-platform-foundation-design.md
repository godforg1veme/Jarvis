...
Prepare Jarvis for future server parsers, monitoring, and
...
- Build generic extension boundaries now; implement concrete parsers, social
  networks, and account adapters later.
...
- stable namespaced action name, for example `files.search` or
  `social.example.publish`;
...
Examples of future use include parser freshness, account-session health, backup
status, and external-account synchronization. Heavy workers have explicit
resource limits so parser or browser workloads cannot starve assistant traffic.
...
5. stable interface contracts for server workers, schedules, monitors, account
   connections, and secret references, without production account-session
   execution.

Concrete account-session transfer, isolated browser infrastructure, and
provider-specific connectors are later projects with separate threat models,
capacity tests, and deployment approval.
