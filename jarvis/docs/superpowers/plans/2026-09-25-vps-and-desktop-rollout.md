...
- [ ] Verify the installed executable path and file version without launching a second copy:
...
- [ ] Compare Windows-client source commit times against the installed artifact date. If the installed app is stale, bump the package patch version to `1.0.1` in `package.json` and `package-lock.json` so the installed build is distinguishable.

### Task 2: Run desktop and Host Agent verification

**Files:** Existing root test scripts and `host-agent/tests/`; no new dependencies.

- [ ] Run the automated Windows-client scripts listed by root `AGENTS.md` from the flattened root:
...
- [ ] Run cloud/desktop Node tests:

```powershell
npm run test:cloud
```

- [ ] Run the Operations UI unit suite, production build, and browser fixture from the flattened root:
...
- [ ] Run Host Agent tests from a Python environment already installed on the machine:
...
- [ ] Confirm local Vosk model and bundled Node runtime assets are present. If a required asset is missing, run the project preparation script instead of copying an unknown build directory.
- [ ] Build the installer:
...
- [ ] Verify the installer filename reports the reviewed package version and inspect its size and timestamp before installation.
- [ ] Run the installer in silent upgrade mode while leaving the installed app data intact:
...
- [ ] Re-read the installed executable’s product version and confirm Jarvis starts. Keep the prior installer until both checks pass.
- [ ] Remove obsolete `dist/` installers only after the new installation is verified; keep the build output untracked.
...
- [ ] Re-read these conditions immediately before deployment. Do not run `git reset`, `git clean`, or pull into the mixed checkout.
- [ ] Inspect the Compose file’s project name, secret-file paths, bind mounts, and named volumes. Keep project name `jarvis-family` and every existing volume/bind path.
- [ ] Verify the database backup destination has enough space and that the resulting backup does not print credentials or database content into the task log.
- [ ] Read the Supervisor flags and recorded owner-acceptance status required by root `AGENTS.md`. Stop before rollout if the running boundary differs from the reviewed state.
...
- [ ] Fetch the published `main` commit and prepare a clean, uniquely named release directory outside the dirty production checkout.
- [ ] Provide Compose only the existing deployment `.env` and explicitly required host secret files; do not copy secrets into the Git worktree or log their values.
- [ ] Run `deploy/scripts/preflight.sh` and the documented one-time database backup from the release tree.
- [ ] Build the new server image and update only the `server` service with project name `jarvis-family`. Leave `postgres`, `gigaam-asr`, `cloudflared`, the DE/NL VPN daemons, and their credentials untouched.
- [ ] Confirm health checks pass, the application reports migration 028 applied, and the public readiness endpoint remains HTTP 200.
- [ ] Run `deploy/scripts/smoke.sh` against the deployed server and inspect only bounded/sanitized log summaries.
- [ ] Update the deployment record with image commit, migration result, smoke result, service state, and rollback location. Do not claim Telegram client acceptance unless a real owner tap was observed.
...
- [ ] Compare hashes for the Host Agent modules and systemd units that define the deployed service against the reviewed `main`; never print credentials or client URIs.
- [ ] If the installed source is equivalent, leave the Host Agent untouched and record its version/hash.
- [ ] If a required code change is missing, run the complete Host Agent unit suite and inspect the deployment script’s backup/rollback behavior before updating that node.
- [ ] Update only the Host Agent service on an affected node. Keep the Xray and Hysteria2 services, firewall rules, VPN clients, probe credentials, and disabled timers unchanged.
- [ ] After any Host Agent update, verify the socket protocol, active service state, VPN health snapshot, and both service uptimes. Do not issue a client, restart VPN, or enable a probe timer.
- [ ] Do not install Docker or Compose on `jarvis-vps-new`.
...
- [ ] Compare the Git SHA in GitHub and the canonical Windows clone. Report any service-specific deployed image/Host Agent hash separately because those installations do not track Git branches.
- [ ] Verify the installed Windows product version, production Compose health, applied database migration, public readiness, and only the Host Agent services that were updated.
- [ ] List all checks that could not run and their concrete reason; do not infer real Telegram or hardware acceptance from automated tests.
