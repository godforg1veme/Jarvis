...
from jarvis_host_agent.hysteria_vpn_manager import hysteria_config, validate_hysteria_state
...
resolved = {item[4][0] for item in socket.getaddrinfo(state['serverName'], 443, family=socket.AF_INET, type=socket.SOCK_DGRAM)}
...
assert actual_config == hysteria_config(state), 'generated config mismatch'
