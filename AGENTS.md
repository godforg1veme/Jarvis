...
- `main` is the only permanent shared branch for this repository.
- Create a short-lived branch or worktree for a focused task. Before merging,
  verify the diff and required tests; after the merge is confirmed, remove the
  task branch from both the local clone and GitHub.
- Never force-push `main`. Before deleting any branch, confirm its useful commits
