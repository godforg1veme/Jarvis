...
const baseSchema = z.object({
  ...
  operationsNlHostKey: z.string().regex(/^[a-z][a-z0-9_-]{0,63}$/),
  operationsNlHostLabel: z.string().min(1).max(100),
  operationsNlSocketPath: z.string().min(1).max(2048),
...
function validateAsr(config) {
  ...
  const privateAsrWorker = url.protocol === 'http:'
    && ['qwen-asr', 'gigaam-asr'].includes(url.hostname)
    && url.port === '8000'
    ...
    && !url.search
    && !url.hash;
  if (config.nodeEnv === 'production' && url.protocol !== 'https:' && !privateAsrWorker) {
...
function loadConfig(env = process.env) {
  const raw = {
    ...
    operationsNlHostKey: String(env.JARVIS_OPERATIONS_NL_HOST_KEY || 'vpn-nl').trim(),
    operationsNlHostLabel: String(env.JARVIS_OPERATIONS_NL_HOST_LABEL || 'Netherlands VPN VPS').trim(),
    operationsNlSocketPath: String(env.JARVIS_OPERATIONS_NL_SOCKET_PATH || '/run/jarvis-host-agent/agent-nl.sock').trim(),
